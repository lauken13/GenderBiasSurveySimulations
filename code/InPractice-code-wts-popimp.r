...
# ACS-NYC 2020 survey data for 18+ individuals
...
# recreate population distribution / impute population values. 
# We split ACS from Female/Male into Female/Male/Other. Based on PT data, 0.5% of the population would be non-binary population. 
# Therefore, we randomly take 0.5% out of the population to create the cells to represent the non-binary population. We split
# 0.5% from female into female/other, and 0.5% from male into male/other. 

acs_ad_female <- acs_ad[which(acs_ad$sex == 2),]
acs_ad_female$ID <- c(1:23920)
acs_ad_fo <- acs_ad_female %>%
  sample_frac(0.005)
acs_ad_female$sex[acs_ad_female$ID %in% acs_ad_fo$ID] <- 3

acs_ad_male <- acs_ad[which(acs_ad$sex == 1),]
acs_ad_male$ID <- c(1:20265)
acs_ad_mo <- acs_ad_male %>%
  sample_frac(0.005)
acs_ad_male$sex[acs_ad_male$ID %in% acs_ad_mo$ID] <- 3

acs_ad <- rbind(acs_ad_female, acs_ad_male)
acs_ad <- rename(acs_ad, gender = sex)
table(acs_ad$gender)
...
RH4_basewt_p <- round(RH4_trimmed,0)
...
dta_path <- save_weights_path(paste0("weights-RH4-nopovgap-population-imputation"), extension = ".dta")
rds_path <- save_weights_path(paste0("weights-RH4-nopovgap-population-imputation"), extension = ".rds")
...
RH4_wts <- read.dta13("RH4/weight/weights-RH4-nopovgap-population-imputation-2022-03-07.dta", convert.factors = FALSE)
